...
/**
 * Class MultiSelect
 * @package SysProd\MultiSelect
 *
 * @property string $addUrl
 * @property string $ajax
 * @property array $ajaxData
 ...
 * @property string $successFunction
 *
...
class MultiSelect extends Widget
{
    ...
    public $defaultLabel = 'Choose items';
...
//$m = [];
//        foreach ( $this->items as $key => $it) {  $m[  Yii::t('app', $key) ]  = $it;     }
//        $this->items = $m;
//        print_r($this->items );
        $list = [$this->defaultIndex => $this->defaultLabel] + $this->items;
        $table = $this->getSelectedItems($this->items);
//        echo iconv('UTF-8', 'ISO-8859-5', $this->defaultLabel);
//        print_r($this->defaultLabel);
//        die();
        $params = [
            'list' => $list,
            'table' => $table,
